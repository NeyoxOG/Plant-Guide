...
function run(args,{stdin,allowFail=false,quiet=false}={}){
  const result=spawnSync(NPX,['--yes','wrangler@latest',...args],{encoding:'utf8',input:stdin,stdio:stdin!==undefined?['pipe','pipe','pipe']:(quiet?['inherit','pipe','pipe']:'inherit')});
...
async function getPassword(){
  if(process.env.PLANT_GUIDE_ADMIN_PASSWORD)return process.env.PLANT_GUIDE_ADMIN_PASSWORD;
  console.log('\nAdmin-Passwort festlegen (mindestens 12 Zeichen).');
  const first=await rl.question('Passwort: ');
  if(first.length<12)throw new Error('Bitte ein Passwort mit mindestens 12 Zeichen verwenden.');
...
function ensureD1(){
  ...
  if(info.status!==0){
    ...
    let created=run(['d1','create',DB_NAME,'--jurisdiction','eu','--binding','DB'],{allowFail:true});
    if(created.status!==0){console.log('EU-Jurisdiction konnte nicht gesetzt werden, zweiter Versuch …');run(['d1','create',DB_NAME,'--binding','DB']);}
...
try{
  ...
  console.log('6/7 Admin-Zugang als Cloudflare Secret setzen …');const password=await getPassword();run(['pages','secret','put','ADMIN_PASSWORD','--project-name',PROJECT,'--config','wrangler.cms.toml'],{stdin:password+'\n'});const nonce=randomBytes(8).toString('hex');console.log(`Secret gesetzt · ${nonce.slice(0,6)}`);
...
}catch(error){console.error('\n✗ Deploy abgebrochen:',error.message);process.exitCode=1}finally{rl.close()}
