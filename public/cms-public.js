(()=>{
  const css=document.createElement('link');css.rel='stylesheet';css.href='cms.css?v=20260908-stable-1';document.head.appendChild(css);
  ...
  function renderShop(products){
    ...
    const nav=document.querySelector('.desktop-nav');if(nav&&!nav.querySelector('a[href="#shop"]')){const contact=nav.querySelector('a[href="#kontakt"]'),link=el('a','','Shop');link.href='#shop';nav.insertBefore(link,contact||null)}const mobile=document.querySelector('.mobile-menu');if(mobile&&!mobile.querySelector('a[href="#shop"]')){const contact=mobile.querySelector('a[href="#kontakt"]'),link=el('a','','Shop');link.href='#shop';mobile.insertBefore(link,contact||null)}
  }

  async function load(){
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),7000);
    try{const res=await fetch('/api/public',{headers:{accept:'application/json'},signal:controller.signal});if(!res.ok)return;const data=await res.json();if(!data?.ok)return;renderPromotions(data.promotions||[]);renderShop(data.products||[])}catch(err){console.info('Plant Guide CMS ist vorübergehend nicht erreichbar.',err)}finally{clearTimeout(timer)}
  }
