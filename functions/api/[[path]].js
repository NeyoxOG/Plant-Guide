...
async function requireBindings(env,{media=false}={}){if(!env.DB)return json({ok:false,error:"D1-Binding DB fehlt. Bitte DB in Cloudflare Pages an plant-guide-db binden und schema.sql ausführen; Details stehen in README.md und wrangler.toml."},503);if(media&&!env.MEDIA)return json({ok:false,error:"R2-Binding MEDIA fehlt. Bitte MEDIA in Cloudflare Pages an plant-guide-media binden; Details stehen in README.md und wrangler.toml."},503);return null}
